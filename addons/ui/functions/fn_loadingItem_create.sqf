 ...
 *
 * Example:
 * [_display, _grp, "stratis"] call ocap_renderterrain_fnc_loadingItem_create;
 *
 * Public: No
 */
...
private _error = [(uiNamespace getVariable ["ocap_renderterrain_errors", []]), _worldName, ""] call (uiNamespace getVariable "BIS_fnc_getFromPairs");

if (_error isNotEqualTo "") exitWith {
	private _item = _display ctrlCreate ['ocap_renderterrain_loadingItem_error', -1, _parentGrp];
...
private _item = _display ctrlCreate ['ocap_renderterrain_loadingItem', -1, _parentGrp];
...
// update states
{
	...
	private _status = [_worldName, _step] call (uiNamespace getVariable "ocap_renderterrain_fnc_stepStatus");
